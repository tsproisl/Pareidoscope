...
die("./06_collect_chunks_fill_db.pl outdir corpus-name dbname chunkdbname regfile") unless(scalar(@ARGV) == 5);
...
sub create_indexes{
    ...
    my $gettypid = $other_dbh->prepare(qq{SELECT types.typid FROM types, gramis, lemmata WHERE types.type=? AND gramis.grami=? AND lemmata.lemma=? AND lemmata.wc=? AND types.gramid=gramis.gramid AND types.lemid=lemmata.lemid});
    my $updatetype = $other_dbh->prepare(qq{UPDATE types SET chunkseq=? WHERE typid=?});
