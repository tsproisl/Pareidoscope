...
die("./08_collect_dependencies.pl outdir corpus-name dbname chunkdbname regfile") unless ( scalar(@ARGV) == 5 );
my $outdir      = shift(@ARGV);
my $corpus      = shift(@ARGV);
my $dbname      = shift(@ARGV);
my $chunkdbname = shift(@ARGV);
...
#my $chunkdbh    = DBI->connect("dbi:SQLite:$outdir/$chunkdbname") or die("Cannot connect: $DBI::errstr");
my $dbh = DBI->connect("dbi:SQLite:$outdir/$dbname") or die("Cannot connect: $DBI::errstr");
open( my $do, ">:encoding(utf8)", "$outdir/dependencies.out" ) or die("Cannot open $outdir/dependencies.out: $!");
&create_indexes;

#$chunkdbh->disconnect();
$dbh->disconnect();
close($do) or die("Cannot close $outdir/dependencies.out: $!");
&common_functions::log( "Finished (Skipped $skipped sentences)", 1, $maxloglevel );
...
sub create_indexes {
    ...
    my $indep         = &get_attribute( "indep", $corpus_handle );
    my $outdep        = &get_attribute( "outdep", $corpus_handle );
    my $Slen          = &get_attribute( "s_len", $corpus_handle );

    # my $insertdependency  = $chunkdbh->prepare(qq{INSERT INTO dependencies (dependency, frequency) VALUES (?, 0)});
    # my $fetchdependencyid = $chunkdbh->prepare(qq{SELECT dependencyid FROM dependencies WHERE dependency = ?});
    # my $insertsentence    = $chunkdbh->prepare(qq{INSERT INTO sentences (cpos, chunkseq, cposseq) VALUES (?, ?, ?)});
    # my $updatefreq        = $chunkdbh->prepare(qq{UPDATE chunks SET frequency = ? WHERE chunkid = ?});
    ...
    # $chunkdbh->do(qq{BEGIN TRANSACTION});
    my %typids;
    my @typids;

    # fill in dependencies
    #foreach my $dependency (@dependencies) {
    #$dependencies{$dependency} = &insertandreturnid( $insertdependency, $fetchdependencyid, [$dependency] );
    #}
    ...
    $cqp->exec("tabulate A match .. matchend word, match .. matchend pos, match .. matchend lemma, match .. matchend wc, match .. matchend indep, match .. matchend outdep, match .. matchend > \"$outdir/tabulate_dependencies.out\"");
    ...
    while ( defined( my $match = <TAB> ) ) {
        chomp($match);
        my ( $words, $poses, $lemmata, $wcs, $indeps, $outdeps, $positions ) = split( /\t/, $match );
        ...
        #next if($ergcounter < 400);
        #last if ( $ergcounter >= 100 );
        if (grep {$a = () = /\|/g; $a > 9} @outdeps) {
	    &common_functions::log("Skipped sentence $ergcounter: " . join(" ", @words), 1, $maxloglevel);
	    $skipped++;
	    next;
        }
        &common_functions::log( $ergcounter, 1, $maxloglevel );
        &common_functions::log( sprintf( "%d/%d (%.2f%%)", $ergcounter, $size, ( ( $ergcounter / $size ) * 100 ) ), 1, $maxloglevel ) unless ( $ergcounter % 100 );
        ...
        die("Different lengths for tabulate and s_len attribute") unless ( $Slen->cpos2str( $positions[0] ) == scalar(@words) );
