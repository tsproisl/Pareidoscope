...
then
    ...
    printf "[%s] %s\n" $(date "+%T") "./01_cwb-encode.sh corpus corpus-outdir registry-file corpus-name outdir"
    exit -1
fi
...
cwb-encode -d $data -f $corpus -R "$registry/$regfile" -xsB -P pos -P lemma -P wc -P indep -P outdep -S s:0+id+len -S adjp:0 -S advp:0 -S conjp:0 -S intj:0 -S lst:0 -S np:0 -S o:0 -S pp:0 -S prt:0 -S qp:0 -S sbar:0 -S ucp:0 -S vp:0 -S h:0 -0 text -0 corpus
