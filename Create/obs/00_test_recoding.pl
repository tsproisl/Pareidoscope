...
my @files = </projects/korpora/BNC-XML/texts/[FGHJK]/*/*.xml>;
#my @files = </projects/korpora/BNC-XML/texts/A/A0/*.xml>;
...
sub test_recoding{
	...
	open(OUT, ">:encoding(iso-8859-1)", "out.tmp") or die("Cannot open out.tmp: $!");
        open(my $PIPE, "cat $file | tr -d '\\n' | sed -e 's/&#10;//g;' | xmllint --format - | xsltproc oneWordPerLine.xsl - |") or die "Can't run xsltproc: $!";
	while(defined(my $line = <$PIPE>)){
	    #$line = Encode::from_to($line, "utf8", "iso-8859-1");
	...
	close(OUT) or die("Cannot close out.tmp: $!");
    }
}
