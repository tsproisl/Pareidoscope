    ...
    while (@$queueref) {
        foreach my $socket (@sockets) {
            ...
            foreach my $record (@localqueue) {
                ...
                my $configuration = $record->[1];
                my $match_length  = $record->[2];
                my @positions     = split /-/xms, $configuration;
                if ( @positions == 1 ) {
		    my $position = $positions[0];
                    ...
                    # can only happen for dependency queries
                    my @parts = unpack "(a${byte_length})*", $marked_ngram;
                    foreach my $position (@positions) {
                        $parts[$position] = '<' . $parts[$position] . '>';
                    }
                    $marked_ngram = join q{}, @parts;
                }
                push( @localresults, [ $marked_ngram, $configuration, $match_length, $record->[3] ] );
            }
